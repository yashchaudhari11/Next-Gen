...
const PHRASE_INTERVAL = 600;
const EXIT_DELAY = PHRASE_INTERVAL * telemetryPhrases.length; // 3600ms
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      setTelemetryIndex((prev) =>
        prev < telemetryPhrases.length - 1 ? prev + 1 : prev
      );
    }, PHRASE_INTERVAL);

    // Dismiss after all phrases have shown ← NEW
    const exitTimer = setTimeout(() => setVisible(false), EXIT_DELAY);

    return () => {
      clearInterval(interval);
      clearTimeout(exitTimer); // ← NEW
    };
  }, []);

  return (
    <AnimatePresence> {/* ← NEW wrapper — enables exit animation */}
      {visible && (
        <motion.div
          key="loader"
          className="fixed inset-0 bg-[#080810] flex flex-col items-center justify-center z-50 select-none"
          exit={{ opacity: 0, filter: "blur(8px)" }} // ← NEW exit: fades + blurs out
          ...
          <div className="relative flex flex-col items-center justify-center space-y-8 max-w-sm w-full p-8 text-center">
            ...
            <div className="relative w-32 h-32 flex items-center justify-center">
              ...
              <motion.div
                ...
                animate={{ scale: [1, 1.1, 1], rotate: [0, 18, -18, 0] }}
