          ...
          {navItems.map((item) => {
            ...
            return (
              <button
                ...
                className={`relative w-full h-11 flex items-center px-3 rounded-[10px] text-[#F0F2FF] cursor-pointer transition-colors duration-200 ${
                  isActive ? "" : "hover:text-[#4F6EF7]"
                }`}
              >
                {/* Active Pill behind */}
                {isActive && (
                ...
                {!isCollapsed ? (
                  <motion.span
                    ...
                    animate={{ opacity: 1, x: 0 }}
                    className={`ml-3 font-sans text-[14px] font-medium relative z-10 ${
                      isActive ? "text-[#F0F2FF]" : "text-[rgba(240,242,255,0.6)]"
                    }`}
                  >
                    {item.label}
                  </motion.span>
                  ...
                  <AnimatePresence>
                    {hoveredItem === item.id && (
                      <motion.span
                        ...
                        className="absolute left-12 bg-[#080810] border border-[rgba(255,255,255,0.08)] px-3 py-1.5 rounded-[6px] text-[12px] font-medium text-[#F0F2FF] shadow-[0_0_15px_rgba(0,0,0,0.5)] z-50 whitespace-nowrap"
                      >
                        {item.label}
                      </motion.span>
                    )}
          ...
          <div className="flex items-center gap-3 overflow-hidden">
            ...
            {!isCollapsed && (
              ...
              >
                ...
                <span className="inline-flex items-center px-1.5 py-0.5 rounded text-[9px] font-bold bg-[#4F6EF7]/15 border border-[#4F6EF7]/30 text-[#4F6EF7] w-fit mt-0.5">
                  PRO PLAN
                </span>
              </motion.div>
            )}
